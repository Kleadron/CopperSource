...
namespace NVorbis
{
    class VorbisStreamDecoder : IVorbisStreamStatus, IDisposable
    {
        ...
        bool LoadComments(DataPacket packet)
        {
            ...
            byte[] vendorData = packet.ReadBytes(packet.ReadInt32());
            _vendor = Encoding.UTF8.GetString(vendorData, 0, vendorData.Length);

            _comments = new string[packet.ReadInt32()];
            for (int i = 0; i < _comments.Length; i++)
            {
                byte[] commentData = packet.ReadBytes(packet.ReadInt32());
                _comments[i] = Encoding.UTF8.GetString(commentData, 0, commentData.Length);
            }
